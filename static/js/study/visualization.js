class Viz {
  ...
  */
  constructor() {
    ...
    // create a tooltip
    this.tooltip = d3.select("#wordcloud")
      .append("div")
    ...
    this.add_interaction_button();
    $("#timeline_header").append('<div id="intersectional"></div>')
    this.install_events();
  }
  ...
  */
  ...
  add_interaction_button() {
    $("#timeline_header").append("<button id='interaction' style='margin-left:10px; cursor:pointer; border-radius: 4px;'>Show Interaction</button>")
  }
  add_binning_option() {
  ...
  clear_intersectional_option (){
    this.intersectional_identity = {};
    $("#intersectional").empty()

  }
  add_intersectional_option(k) {
    if (!(k in this.intersectional_identity) ){
      this.intersectional_identity[k] = 1;
      $("#intersectional").append('<span><input identity="'+k+'" class="intersectional_option" style="margin-left:10px" type="checkbox" checked> ' +
        '<label for="aggregate">'+k+'</label></span>');
    }
  }
  ...
  */
      ...
      data.forEach(function(d) {
        if(character_dict[d.name] == CONFIG['character_state']['active']) {
          ...
          for (let k in obj.demographics) {
            if (k in self.intersectional_identity)
  ...
  /*
   ---------------------- WORDCLOUD -------------------------
  */
  get_wordcloud_data(update = false, kind = "both"){
    var self = this;
    $.ajax({
      url: '/study_wordcloud',
      data: JSON.stringify({kind:kind, story_name:name}),
      type: 'POST',
  ...
  get_paired_distance(groups){
    if(groups.length != 2)
      return;

    $.ajax({
      url: '/calculate_distance',
      data: JSON.stringify({ grp1: groups[0][2], grp2: groups[1][2] }),
      type: 'POST',
      success: function (res) {
        console.log(res)
      },
  ...
  update_wordcloud(data, option=false) {
    ...
    let compare_groups = groups.map(([character, category, words]) => [
      character,
      category,
      words.map(w => w.word)
    ]);
    console.log("groups: ", compare_groups)

    groups = groups.map(([character, category , words]) => [
        character,
        category,
    ...
    // console.log("df: ", df)
    self.get_paired_distance(compare_groups);
  ...
  */
  get_graph_data(update = false){
    ...
    $.ajax({
      url: '/study_get_graph',
      type: 'POST',
      data: JSON.stringify({story_name: name, alias_dict:CHARACTER.alias_dict, characters: CHARACTER.characters.map(d => d.name)}),
      success: function(res){
        console.log(res)
  ...
  reset_intersectional_option() {
    var self = this;
    let characters = CHARACTER.characters;
    let option = $("#show_dropdown").val()

    if(option == "intersectional"){
      characters.forEach(function (c) {
        for (let k in c.demographics) {
          self.add_intersectional_option(k);
        }
      })

      $(".intersectional_option").on("change", function () {
        let k = $(this).attr("identity")
        if ($(this).prop("checked")) {
          self.intersectional_identity[k] = 1;
        }
        else {
          delete self.intersectional_identity[k];
        }
  ...
  */
  install_events () {
    ...
    $("#interaction").on("click", function() {self.update_network("None")});
    ...
    $("#show_dropdown").on("change", function(){
      self.clear_intersectional_option();
      self.reset_intersectional_option();
      self.update_timeline()
      self.selected = [];
    ...
    //     console.log(d3.select(e.currentTarget).attr('x'))
    //     // return self.tooltip.style("visibility", "visible");
    //     self.tooltip
    //       .style("opacity", 1)
    //       .html(d.name)
    //       .style("left", (d3.select(e.currentTarget).attr('x') + 50) + "px")
    //       .style("top", (d3.select(e.currentTarget).attr('y') + 20) + "px")
    //   })
    //   .on("mousemove", function (e, d) {
    //     // return self.tooltip.style("top", (e.pageY - 10) + "px").style("left", (e.pageX + 10) + "px");
    //   })
